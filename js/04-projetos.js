...
function projectSave(p){
  DB.update('projetos',p.id,{cotacoes:p.cotacoes,ordensCompra:p.ordensCompra,documentosProjeto:p.documentosProjeto,pendencias:p.pendencias,docsApae:p.docsApae,docsFornecedor:p.docsFornecedor,empresas:p.empresas,pagamentos:p.pagamentos,plano:p.plano,documentosRecurso:p.documentosRecurso,movimentacoes:p.movimentacoes,arquivado:p.arquivado});
}
...
function openFormProjeto(id, opts={}){
  ...
  document.getElementById('formProjeto').onsubmit=e=>{
    ...
    if(item){
      ...
      const novo={id:DB.nextId('PRJ','projeto'),codigo:'PRJ-'+String(DB.getConfig().counters.projeto).padStart(4,'0'),...dados,criadoEm:Date.now(),atualizadoEm:Date.now(),cotacoes:[],ordensCompra:[],documentosProjeto:[],pendencias:[],docsApae:[],docsFornecedor:[],empresas:[],pagamentos:[],plano:{descricao:'',anexo:null},documentosRecurso:[],movimentacoes:[],arquivado:false};
