/* ---------------------------------------------------------
   12. ATIVIDADES / SOLICITAÇÕES
   --------------------------------------------------------- */
function tipoAtividadeIcon(tipo){
  return tipo === 'Tarefa' ? '📝' : tipo === 'Lembrete recorrente' ? '🔄' : '📋';
}
function tipoAtividadeTom(tipo){
  return tipo === 'Tarefa' ? 'primary' : tipo === 'Lembrete recorrente' ? 'warn' : 'neutral';
}
...
function prazoAtividade(s){
  ...
  if (recorrente){
    // A próxima ocorrência só é empurrada para o futuro quando o ciclo atual
    // já foi concluído (ver concluirAtividade). Enquanto isso, ela fica
    // "Concluída" (verde) e só volta a valer no dia da próxima ocorrência.
    // Caso a recorrência estivesse atrasada e o "fiz hoje" tenha compensado o
    // atraso caindo exatamente em hoje, `ultimaConclusao` garante que ainda
    // assim apareça como concluída (evita reabrir no mesmo dia).
    if (d > 0 || (d === 0 && s.ultimaConclusao === todayISO())) return {tom:'ok',texto:`Concluída · volta em ${formatDateBR(prazo)}`,data:prazo};
...
function badgeStatusSolicitacao(status){
  const map = {'Pendente':'neutral','Em andamento':'primary','Aguardando':'warn','Concluída':'ok','Cancelada':'neutral'};
  return map[status] || 'neutral';
}
function prioridadePeso(p){ return ({Urgente:4,Alta:3,Normal:2,Baixa:1}[p] || 0); }

function renderSolicitacoes(){
  const filtros=getFiltrosValores('filtrosSolicitacoes'); let lista=DB.getAll('solicitacoes').filter(s=>!ehTarefaRenovacaoDocumento(s));
  if(filtros.busca){const q=filtros.busca.toLowerCase(); lista=lista.filter(s=>[s.titulo,s.descricao,s.responsavel,s.categoria].join(' ').toLowerCase().includes(q));}
  if(filtros.frequencia) lista=lista.filter(s=>(s.recorrencia?.frequencia||'Única')===filtros.frequencia);
  if(filtros.prioridade) lista=lista.filter(s=>s.prioridade===filtros.prioridade);
  if(filtros.periodo==='hoje') lista=lista.filter(s=>prazoAtividade(s).data===todayISO());
  if(filtros.periodo==='atrasadas') lista=lista.filter(s=>solicitacaoAtrasada(s));
  if(filtros.periodo==='proximas') lista=lista.filter(s=>{const d=prazoAtividade(s).data; return d && daysDiffFromToday(d)>0;});
  lista.sort((a,b)=>{if(filtros.ordenacao==='recente')return(b.criadoEm||0)-(a.criadoEm||0);if(filtros.ordenacao==='prioridade')return prioridadePeso(b.prioridade)-prioridadePeso(a.prioridade);const da=prazoAtividade(a).data?parseISODate(prazoAtividade(a).data).getTime():Infinity;const db_=prazoAtividade(b).data?parseISODate(prazoAtividade(b).data).getTime():Infinity;return da-db_;});
  const todas=DB.getAll('solicitacoes').filter(s=>!ehTarefaRenovacaoDocumento(s)); const abertas=todas.filter(s=>s.status!=='Concluída'&&s.status!=='Cancelada');
  const hoje=abertas.filter(s=>prazoAtividade(s).data===todayISO()).length; const atrasadas=abertas.filter(s=>solicitacaoAtrasada(s)).length; const urg=todas.filter(s=>s.prioridade==='Urgente'&&s.status!=='Concluída'&&s.status!=='Cancelada').length; const rec=todas.filter(s=>(s.recorrencia?.frequencia||'Única')!=='Única').length;
  document.getElementById('resumoAtividades').innerHTML=[['Para hoje',hoje,'danger'],['Atrasadas',atrasadas,'danger'],['Urgentes',urg,'warn'],['Recorrentes',rec,'primary']].map(x=>`<div class="activity-summary-card"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join('');
  const box=document.getElementById('listaAtividades'); document.getElementById('vazioSolicitacoes').hidden=lista.length!==0;
  box.innerHTML=lista.map(s=>{const pz=prazoAtividade(s);const freq=s.recorrencia?.frequencia||'Única';const recorr=freq!=='Única';const jaConcluida=pz.tom==='ok';return `<article class="activity-card ${solicitacaoAtrasada(s)?'is-late':''} ${jaConcluida?'is-done':''}" data-id="${s.id}"><div class="activity-main"><div class="activity-title-row"><span class="activity-icon">${recorr?'🔄':'📝'}</span><div><div class="activity-title">${escapeHTML(s.titulo)}</div><div class="activity-meta"><span class="badge-pill badge-${recorr?'warn':'primary'}">${freq}</span>${s.responsavel?`<span>👤 ${escapeHTML(s.responsavel)}</span>`:''}</div></div></div><div class="activity-description">${escapeHTML(s.descricao||'Sem descrição')}</div></div><div class="activity-side"><div>${badgeHTML(badgePrioridade(s.prioridade),s.prioridade)}</div><div>${badgeHTML(pz.tom,pz.texto)}</div></div><div class="activity-actions">${jaConcluida?'':`<button class="btn btn-sm btn-primary" data-act="concluir">${recorr?'✓ Fiz hoje':'✓ Concluir'}</button>`}<button class="btn btn-sm btn-primary" data-act="ver">Acessar</button><button class="btn btn-sm" data-act="editar">Editar</button><button class="btn btn-sm btn-danger" data-act="excluir">Excluir</button></div></article>`;}).join('');
  box.querySelectorAll('.activity-card').forEach(card=>{const id=card.dataset.id;card.querySelector('[data-act="ver"]').onclick=()=>abrirDetalheSolicitacao(id);card.querySelector('[data-act="editar"]').onclick=()=>openFormSolicitacao(id);const btnConcluir=card.querySelector('[data-act="concluir"]');if(btnConcluir)btnConcluir.onclick=()=>concluirAtividade(id);card.querySelector('[data-act="excluir"]').onclick=()=>confirmAction('Tem certeza que deseja excluir esta tarefa?',()=>{const item=DB.getById('solicitacoes',id);DB.remove('solicitacoes',id);registrarHistorico({modulo:'secretaria',acao:'exclusão',descricao:`Tarefa "${item.titulo}" excluída.`,refId:id});showToast('Tarefa excluída.');renderSolicitacoes();});});
}
document.querySelectorAll('#filtrosSolicitacoes [data-filter]').forEach(el=>el.addEventListener('input',renderSolicitacoes));
document.querySelector('[data-action="nova-solicitacao"]').addEventListener('click',()=>openFormSolicitacao());

function concluirAtividade(id){
  const item=DB.getById('solicitacoes',id);if(!item)return;const freq=item.recorrencia?.frequencia||'Única';
  if(freq!=='Única'){
    const atual=item.recorrencia?.proxima||item.prazo||todayISO();
    // Já concluída neste ciclo: ou a próxima ocorrência já está no futuro,
    // ou o "fiz hoje" já foi registrado hoje (evita compensar atraso e cair em "hoje" de novo).
    if(daysDiffFromToday(atual)>0 || item.ultimaConclusao===todayISO()){showToast(`Esta tarefa já foi concluída. Volta em ${formatDateBR(atual)}.`);return;}
...
function openFormSolicitacao(id){
  const item=id?DB.getById('solicitacoes',id):null; const rec=item?.recorrencia||{};
  openModal(item?'Editar tarefa':'Nova tarefa',`<form id="formSolicitacao" novalidate><div class="form-grid">
    <div class="field full"><label for="f_titulo">O que precisa ser feito? *</label><input class="input" id="f_titulo" required value="${escapeHTML(item?.titulo||'')}"></div>
    <div class="field"><label for="f_prioridade">Prioridade</label><select class="input" id="f_prioridade">${['Baixa','Média','Alta','Urgente'].map(x=>`<option ${(item?.prioridade||'Média')===x?'selected':''}>${x}</option>`).join('')}</select></div>
    <div class="field"><label for="f_frequencia">Repetição</label><select class="input" id="f_frequencia">${['Única','Diária','Semanal','Mensal','Anual'].map(x=>`<option ${(rec.frequencia||'Única')===x?'selected':''}>${x}</option>`).join('')}</select></div>
    <div class="field"><label for="f_prazo">Primeiro dia *</label><input class="input" type="date" id="f_prazo" required value="${item?.prazo||todayISO()}"></div>
    <div class="field"><label for="f_horario">Horário (opcional)</label><input class="input" type="time" id="f_horario" value="${escapeHTML(rec.horario||'')}"></div>
    <div class="field full" id="recorrenciaFields"><div class="recurrence-box"><strong>🔄 Configuração da repetição</strong><div class="form-grid" style="margin-top:10px"><div class="field" id="campoDiaSemana"><label for="f_diaSemana">Dia da semana</label><select class="input" id="f_diaSemana">${['Domingo','Segunda-feira','Terça-feira','Quarta-feira','Quinta-feira','Sexta-feira','Sábado'].map((x,i)=>`<option value="${i}" ${Number(rec.diaSemana??1)===i?'selected':''}>${x}</option>`).join('')}</select></div><div class="field" id="campoDiaMes"><label for="f_diaMes">Dia do mês</label><input class="input" type="number" id="f_diaMes" min="1" max="31" value="${rec.diaMes||new Date().getDate()}"></div></div></div></div>
    <div class="field"><label for="f_responsavel">Responsável</label><input class="input" id="f_responsavel" value="${escapeHTML(item?.responsavel||'')}" placeholder="Ex.: Secretaria"></div>
    <div class="field"><label for="f_categoria">Categoria</label><input class="input" id="f_categoria" value="${escapeHTML(item?.categoria||'')}" placeholder="Ex.: administrativo"></div>
    <div class="field full"><label for="f_descricao">Descrição / observações</label><textarea id="f_descricao">${escapeHTML(item?.descricao||'')}</textarea></div>
    <div class="field full"><label>🔗 Vincular a outros registros</label><div class="relacionados-form-section">${item?renderSelectorRelacionados('solicitacao',item.id,'projeto'):'<p class="empty-inline">Salve a tarefa primeiro para adicionar vínculos.</p>'}${item?renderSelectorRelacionados('solicitacao',item.id,'empresa'):''}</div><small class="muted">Você pode vincular esta tarefa a projetos e empresas.</small></div>
  ...
  const freqEl=document.getElementById('f_frequencia'), recBox=document.getElementById('recorrenciaFields'), semana=document.getElementById('campoDiaSemana'), mes=document.getElementById('campoDiaMes');
  function toggleRec(){const f=freqEl.value;recBox.style.display=f==='Única'?'none':'block';semana.style.display=f==='Semanal'?'block':'none';mes.style.display=f==='Mensal'?'block':'none';} freqEl.addEventListener('change',toggleRec);toggleRec();
  document.getElementById('btnCancelarForm').onclick=closeModal;
  document.getElementById('formSolicitacao').addEventListener('submit',e=>{e.preventDefault();const titulo=document.getElementById('f_titulo').value.trim();if(!titulo){const er=document.getElementById('formErro');er.hidden=false;er.textContent='Informe o que precisa ser feito.';return;}const freq=freqEl.value;const prazo=document.getElementById('f_prazo').value;const recDados=freq==='Única'?null:{frequencia:freq,horario:document.getElementById('f_horario').value||null,diaSemana:Number(document.getElementById('f_diaSemana').value),diaMes:Number(document.getElementById('f_diaMes').value)||new Date().getDate(),proxima:item?.recorrencia?.proxima||prazo};const dados={titulo,tipo:'Tarefa',prioridade:document.getElementById('f_prioridade').value,descricao:document.getElementById('f_descricao').value.trim(),responsavel:document.getElementById('f_responsavel').value.trim(),categoria:document.getElementById('f_categoria').value.trim(),prazo,status:item?.status==='Concluída'?'Pendente':'Pendente',recorrencia:recDados};if(item){DB.update('solicitacoes',item.id,dados);registrarHistorico({modulo:'secretaria',acao:'edição',descricao:`Tarefa "${dados.titulo}" editada.`,refId:item.id});showToast('✓ Tarefa atualizada.');}else{const novo={id:DB.nextId('SOL','solicitacao'),...dados,anexos:[],criadoEm:Date.now(),atualizadoEm:Date.now()};DB.insert('solicitacoes',novo);registrarHistorico({modulo:'secretaria',acao:'criação',descricao:`Tarefa "${novo.titulo}" criada (${freq}).`,refId:novo.id});showToast('✓ Tarefa criada.');}closeModal();renderCurrentView();});
  if(item) processarRelacionadosEmForm('solicitacao',item.id,'formSolicitacao');
}

function abrirDetalheSolicitacao(id){
  const s=DB.getById('solicitacoes',id); if(!s)return; const pz=prazoAtividade(s); const hist=DB.getAll('historico').filter(h=>h.refId===id);
  openModal(`${tipoAtividadeIcon(s.tipo||'Solicitação')} ${s.id}`,`<div class="activity-detail-head"><div><span class="badge-pill badge-${tipoAtividadeTom(s.tipo||'Solicitação')}">${escapeHTML(s.tipo||'Solicitação')}</span><h2>${escapeHTML(s.titulo)}</h2></div><div>${badgeHTML(badgeStatusSolicitacao(s.status),s.status)}</div></div>
    <div class="form-grid"><div class="detail-block"><div class="detail-label">Prioridade</div><div class="detail-value">${badgeHTML(badgePrioridade(s.prioridade),s.prioridade)}</div></div><div class="detail-block"><div class="detail-label">Prazo</div><div class="detail-value">${badgeHTML(pz.tom,pz.texto)}</div></div><div class="detail-block"><div class="detail-label">Responsável</div><div class="detail-value">${escapeHTML(s.responsavel||'—')}</div></div><div class="detail-block"><div class="detail-label">Solicitante</div><div class="detail-value">${escapeHTML(s.solicitante||'—')}</div></div><div class="detail-block"><div class="detail-label">Projeto</div><div class="detail-value">${escapeHTML(s.projetoId||'Nenhum')}</div></div><div class="detail-block"><div class="detail-label">Categoria</div><div class="detail-value">${escapeHTML(s.categoria||'—')}</div></div></div>
    ${(s.recorrencia?.frequencia&&s.recorrencia.frequencia!=='Única')?`<div class="recurrence-detail">🔄 <strong>Recorrência ${escapeHTML(s.recorrencia.frequencia.toLowerCase())}</strong> · Próxima ocorrência: ${formatDateBR(s.recorrencia?.proxima||proximaRecorrencia(s))}${s.recorrencia?.horario?' · '+s.recorrencia.horario:''}</div>`:''}
    <div class="detail-block"><div class="detail-label">Descrição</div><div class="detail-value">${escapeHTML(s.descricao||'—')}</div></div><div class="detail-block"><div class="detail-label">Observações</div><div class="detail-value">${escapeHTML(s.observacoes||'—')}</div></div>
    ${subtarefasHTML('solicitacoes',s)}
    <div class="detail-block"><div class="detail-label">Histórico</div><div class="hist-timeline">${hist.length?hist.map(h=>`<div><span class="muted" style="font-family:var(--font-mono)">${timestampToBR(h.timestamp)}</span><br>${escapeHTML(h.descricao)}</div>`).join(''):'<span class="muted">Sem eventos registrados.</span>'}</div></div>
    <div class="modal-actions"><button class="btn btn-ghost" id="btnFecharDetalhe">Fechar</button><button class="btn btn-primary" id="btnEditarDetalhe">Editar</button></div>`);
  document.getElementById('btnFecharDetalhe').onclick=closeModal; document.getElementById('btnEditarDetalhe').onclick=()=>openFormSolicitacao(id);
  bindSubtarefasEvents('solicitacoes',id,()=>{closeModal();abrirDetalheSolicitacao(id);renderCurrentView();});
}
...
function eventosAgendaCompletos(){
  ...
  const tarefas=DB.getAll('solicitacoes').filter(s=>prazoAtividade(s).data && !ehTarefaRenovacaoDocumento(s)).map(s=>({
    ...
    horarioInicio:s.recorrencia?.horario||'', horarioFim:'', local:'', responsavel:s.responsavel||'',
